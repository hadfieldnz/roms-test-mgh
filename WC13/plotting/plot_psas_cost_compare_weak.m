%
% PLOT_PSAS_COST_COMPARE_WEAK:  Plot 4D-PSAS cost function
%
...
Inp1='../PSAS/EX3_CONGRAD/wc13_mod.nc';           % W4D-PSAS default
...
Inp5='../PSAS/EX4/wc13_mod.nc';                   % W4D-PSAS weak constraint
...
% Weak constraint PSAS with RPCG

Jo5 =nc_read(Inp5,'Jobs',[],NaN);        % observation cost function
Jb5 =nc_read(Inp5,'Jb',[],NaN);          % background cost function
J5  =nc_read(Inp5,'Jact',[],NaN);        % total cost function
JNL5=nc_read(Inp5,'NL_fDataPenalty');    % final NL model data penalty
...
Jo5=Jo5./2;                              % Need to divide by 2
Jb5=Jb5./2;                              % because of definition
J5=J5./2;                                % of penalty functional
JNL5=JNL5./2;
...
if (SINGLE_PLOT),
  ...
  plot(log10(squeeze(J5(2:end))),'c:','LineWidth',2);
  plot(log10(squeeze(Jp(:))),'s','LineWidth',1, ...
       'MarkerEdgeColor','k','MarkerFaceColor','k','MarkerSize',6);

  axis([1 Niter*Nouter-1 ymax max(log10(J1(:)))]);

  legend('PSAS','MINRES','RPCG','Weak RPCG','I4DVAR','Location','Northeast');
  title(['WC13: Cost Functions, outer loops = ', num2str(1:Nouter)]);
  ylabel('log_{10}(J)');
  ...
  if (PRINT),
    print -dpng -r300 plot_psas_cost_weak.png
  end
